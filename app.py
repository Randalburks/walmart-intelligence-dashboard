...
""")
    ...
    st.markdown("#### 4) Behavioral signals")
    cL, cR = st.columns(2)
    with cL:
        ts_all = panel.groupby("date", as_index=False)["sales"].sum()
        st.plotly_chart(px.line(ts_all, x="date", y="sales",
                                title="All-sampled items — daily sales"),
    ...
    with cR:
        if "event_type_1" in panel.columns:
            ...
            st.plotly_chart(px.bar(ev, x="event_type_1", y="sales",
                                   title="Sales by event type (top 12)"),
                            use_container_width=True)
    st.markdown("""
**What the results mean.** The daily timeline sets the baseline volatility and reveals weekly rhythm or shocks; the event bar ranks which event types matter in this sample.  
**How this drives the dashboard.** Clear weekly rhythm justifies the seasonal baseline in the Forecast tab. Event impact motivates the date overlays and future feature engineering.
""")

    st.markdown("#### 5) Imputation diagnostics (sell_price)")
...
""")
    ...
    st.markdown("#### 6) Quick diagnostic insights (auto)")
    insights = []
